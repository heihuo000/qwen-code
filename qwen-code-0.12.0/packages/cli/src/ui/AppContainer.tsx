...
export const AppContainer = (props: AppContainerProps) => {
  ...
  // Set up the query callback for scheduled tasks (after hooks are initialized)
  useEffect(() => {
    if (isConfigInitialized) {
      const scheduler = config.getSchedulerService();
      scheduler.setQuerySubmitCallback(async (query: string) => {
        // This callback will be called by scheduled tasks to submit queries to Qwen
        // We use the same flow as user input: add to message queue
        addMessage(query);
      });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [config, isConfigInitialized, addMessage]);
