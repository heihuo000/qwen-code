...
if (!existsSync(join(root, 'node_modules'))) {
  // On Android platform, skip postinstall scripts to avoid @vscode/vsce-sign build failure
  const isAndroid = process.platform === 'android';
  if (isAndroid) {
    console.log('Detected Android platform, installing with --ignore-scripts to skip problematic postinstall...');
...
// 7. vscode-ide-companion (depends on webui) - skip on Android
const isAndroid = process.platform === 'android';
const buildOrder = [
  'packages/test-utils',
  'packages/core',
...
// Only build VSCode extension if not on Android platform
if (!isAndroid) {
  buildOrder.push('packages/vscode-ide-companion');
}
